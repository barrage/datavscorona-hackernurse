...
import bedListExporterFields from '@/modules/bed/bed-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (!bedListExporterFields || !bedListExporterFields.length) {
          throw new Error('bedListExporterFields is required');
        }
        ...
        const response = await BedService.list(
          filter,
          getters.orderBy,
          null,
          null,
