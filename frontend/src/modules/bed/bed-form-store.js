  ...
  actions: {
    ...
    async doCreate({ commit }, values) {
      try {
        ...
        await BedService.create(values);
        commit('CREATE_SUCCESS');
        Message.success(i18n('entities.bed.create.success'));
        routerAsync().push('/bed');
      } catch (error) {
        Errors.handle(error);
        commit('CREATE_ERROR');
    ...
    async doUpdate({ commit }, { id, values }) {
      try {
        ...
        await BedService.update(id, values);

        commit('UPDATE_SUCCESS');
        Message.success(i18n('entities.bed.update.success'));
        routerAsync().push('/bed');
      } catch (error) {
        Errors.handle(error);
        commit('UPDATE_ERROR');
