  ...
  actions: {
    ...
    async doCreate({ commit }, values) {
      try {
        ...
        await HospitalService.create(values);
        commit('CREATE_SUCCESS');
        Message.success(i18n('entities.hospital.create.success'));
        routerAsync().push('/hospital');
      } catch (error) {
        Errors.handle(error);
        commit('CREATE_ERROR');
    ...
    async doUpdate({ commit }, { id, values }) {
      try {
        ...
        await HospitalService.update(id, values);

        commit('UPDATE_SUCCESS');
        Message.success(i18n('entities.hospital.update.success'));
        routerAsync().push('/hospital');
      } catch (error) {
        Errors.handle(error);
        commit('UPDATE_ERROR');
