...
import respiratorListExporterFields from '@/modules/respirator/respirator-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (!respiratorListExporterFields || !respiratorListExporterFields.length) {
          throw new Error('respiratorListExporterFields is required');
        }
        ...
        const response = await RespiratorService.list(
          filter,
          getters.orderBy,
          null,
          null,
