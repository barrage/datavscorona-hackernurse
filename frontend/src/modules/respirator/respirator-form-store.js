  ...
  actions: {
    ...
    async doCreate({ commit }, values) {
      try {
        ...
        await RespiratorService.create(values);
        commit('CREATE_SUCCESS');
        Message.success(i18n('entities.respirator.create.success'));
        routerAsync().push('/respirator');
      } catch (error) {
        Errors.handle(error);
        commit('CREATE_ERROR');
    ...
    async doUpdate({ commit }, { id, values }) {
      try {
        ...
        await RespiratorService.update(id, values);

        commit('UPDATE_SUCCESS');
        Message.success(i18n('entities.respirator.update.success'));
        routerAsync().push('/respirator');
      } catch (error) {
        Errors.handle(error);
        commit('UPDATE_ERROR');
