...
class AuthService {
    ...
    static async signup(email, password, options = {}) {
        ...
        const hospital = options.body.hospital;

        if (!hospital) {
            throw new ValidationError(
                options.language,
                'auth.hospitalRequired'
            );
        }
        ...
        const userData = {
             ...
             */
            roles: isFirstUser ? [Roles.values.owner] : [Roles.values.bedEditor, Roles.values.respiratorEditor],
        };

        // if hospital already exists, attach to new user
        const hospitalData = {name: hospital};
        const hospitalFromDatabase = await new HospitalRepository().findByName(hospital);
        if (hospitalFromDatabase) {
            userData.hospital = hospitalFromDatabase.id;
        }

        // create user
        const newUser = await UserRepository.createFromAuth(userData);

        // create hospital if doesnt exist and attach the hospital id to newly created user
        if (!hospitalFromDatabase) {
            try {
                const newHospital = await new HospitalService({
                    currentUser: newUser,
                    language: options.language
                }).create(hospitalData);
                userData.hospital = newHospital.id;
                const updateUser = await UserRepository.update(newUser.id, userData);
            } catch (e) {
                console.log(`Cannot create hospital with data ${JSON.stringify(hospitalData, null, 2)}`);
